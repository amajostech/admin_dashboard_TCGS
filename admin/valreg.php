...
if(mysqli_num_rows($chkexist) >= 1) {
	...
	$insert = mysqli_query($link, "INSERT INTO `stud_profile` (`id`, `school_id`, `stud_name`, `phone`, `class`, `reg_no`, `gender`, `religion`, `homeadd`, `stateorigin`, `lga`, `pob`, `email`, `bio`, `category`, `dob`, `passport`) VALUES (NULL, '$adminschid', '$studnm', '$studcon', '$studcla', '$regno', '$gender', '$religion', '$homeadd', '$stateorigin', '$lga', '$pob', '$studem', '$bio', '$studcat', '$studdob', '$filename' ); ");
	
      if ($insert) {
      	echo "<script> alert('Pupil has been added. Passport has been Uploaded.');
		            window.location.href='add-student.php'; </script>";
      }else{
